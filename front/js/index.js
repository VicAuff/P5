...
fetch("http://localhost:3000/api/products") // Recupere les donnees de l'API
  ...
  .then((product) => {
    ...
    let link = document.querySelector('#items').appendChild(document.createElement('a'));
...
fetch(`http://localhost:3000/api/products/${id}`) // Requete GET API pour la recup plus haut 
  ...
  .then((product) => {
    ...
    document.querySelector(".item__img").innerHTML = `<img src="${product.imageUrl}" alt="${product.name}">`; // Injection dans HTML de img pour class item__img + alt

    //Insertion des couleurs
    
    for(let color of product.colors){ // Iteration API des object utilisant proprietes 'colors'
